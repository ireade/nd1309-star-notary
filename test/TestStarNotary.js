...
it('can add the star name and star symbol properly', async() => {
    ...
    const newStarSymbol = "IRE";

    await instance.createStar(newStarName, newStarSymbol, tokenId, {from: accounts[0]});

    const newStar = await instance.tokenIdToStarInfo.call(tokenId);

    assert.equal(newStar.name, newStarName);
    assert.equal(newStar.symbol, newStarSymbol);
});
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Star 1', 'S1', star1Id, {from: owner1});
    await instance.createStar('Star 2', 'S2', star2Id, {from: owner2});
...
it('lets a user transfer a star', async() => {
    ...
    await instance.createStar('Star 1', 'S1', starId, { from: owner1 });
